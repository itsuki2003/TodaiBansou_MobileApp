...
import { XMarkIcon, ClockIcon, BellIcon, CalendarIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline';
import { NotificationWithDetails, NotificationFormData, NotificationCategory } from '@/types/notifications';

interface NotificationEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  notification: NotificationWithDetails | null;
  onSubmit: (notificationId: string, updateData: Partial<NotificationFormData>) => Promise<void>;
...
export default function NotificationEditModal({
  ...
  // notificationが変更されたときにフォームデータを初期化
  useEffect(() => {
    if (notification) {
      setFormData({
  ...
  const updateFormData = (field: keyof NotificationFormData, value: any) => {
    setFormData(prev => {
      ...
      if (field === 'is_immediate_publish' && value === true) {
        ...
        newData.status = notification?.status || '下書き';
        newData.publish_timestamp = notification?.publish_timestamp || '';
      }
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.publish_timestamp && formData.status === '下書き') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!notification || !validateForm()) return;
  ...
  const setDefaultPublishTime = () => {
    ...
    updateFormData('publish_timestamp', tomorrow.toISOString());
  };

  if (!notification) return null;

  const isPublished = notification.status === '配信済み';
  const canEditPublishTime = !isPublished;
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
                    ...
                    <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className={`font-medium ${
                          ...
                          notification.is_scheduled ? 'text-purple-600' : 
                          'text-yellow-600'
                        }`}>
                          {isPublished ? '配信済み' : 
                           notification.is_scheduled ? '予約配信' : 
                           '下書き'}
                        </span>
                      </div>
                      {notification.is_scheduled && notification.time_until_publish && (
                        <div className="mt-1 text-xs text-purple-600">
                          配信まで約{
                            notification.time_until_publish > 60 
                              ? `${Math.ceil(notification.time_until_publish / 60)}時間`
                              : `${notification.time_until_publish}分`
                          }
                        </div>
                      )}
                    </div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                          ...
                          {!formData.is_immediate_publish && (
                            <div>
                              ...
                              <div className="flex space-x-2">
                                <div className="flex-1 relative">
                                  ...
                                  <input
                                    ...
                                    value={formatDateTimeLocal(formData.publish_timestamp)}
                              ...
                              {formData.publish_timestamp && !formData.is_immediate_publish && !isPublished && (
                      ...
                      {isPublished && (
                        <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                          <div className="flex">
                            ...
                            <div className="ml-3">
                              <h4 className="text-sm font-medium text-orange-800">配信済みのお知らせ</h4>
                              <p className="text-sm text-orange-700 mt-1">
                                このお知らせはすでに配信されています。内容とカテゴリーのみ編集できます。
                              </p>
                            </div>
                          </div>
                        </div>
                ...
                <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                  ...
                  >
                    {submitting ? (
                      formData.is_immediate_publish ? '配信中...' : '更新中...'
                    ) : (
                      formData.is_immediate_publish ? '今すぐ配信' : '更新'
                    )}
                  </button>
                  <button
                    type="button"
